...
//           now at EPFL, Switzerland assyr.abdulle@epfl.ch
//
//   Version of April 2002  minor bug corrected
//   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//   PLEASE CITE THE FOLLOWING PAPER WHEN USING ROCK2:
//
//   [ ] A. Abdulle & A.A. Medovikov
//       Second order Chebyshev methods based on
//       orthogonal polynomials
//       Numer. Math.  90, no. 1  (2001), 1--18.
//       
//     
//    Input parameters  
//    ----------------  
...
//
//    *f:         Pointer to a function computing the value 
//                of f(t,y). Must have the form.
//
//                  void f(const int neqn, const real t, const real y[], real f[])
//                  {
//                      f[0]=...
//                      ...
//                      f[neqn-1]=...
...
//    *rtol       can be both a pointer to a scalar or to
//                arrays of length neqn (double precision).
//
//    rho:        Pointer to a function giving the spectral
//                radius of the Jacobian matrix  of f at (t,y).
//                Must have the form
//                
//                  real rho(const int neqn, const real t, const real y[])
//                  {
//                      rho=... 
//                      return rho;
//                  }
...
//              
//    iwork[0]:   =0 ROCK2 attempts to compute the spectral radius 
//                   internally. Define a dummy function rho.
//
//                =1 rho returns an upper bound of the spectral 
//                   radius  of the Jacobian matrix of f at (t,y).
//   
//    iwork[1]:   =0 The Jacobian is not constant.                  
//                =1 The Jacobian is constant,
//                   the function rho is called only once.
//
//    iwork[2]:   =0 Return and solution at tend
...
//                   any arguments.
//
//    iwork[3]:   =0 Atol and rtol are scalar.
//                =1 Atol and rtol are array of length neqn.
//
//    work[] :      Workspace of length 5*neqn if iwork[0]=0,
//                  otherwise of length 4*neqn.
//                  work[0],..,work[4*neqn-1] serve as
//                  working space for the solution of
//                  the ode.
//                  work[4*neqn],..,work[5*neqn-1]
...
//
//    iwork[4]    =Number of function evaluations.
//    iwork[5]    =Number of steps.
//    iwork[6]    =Number of accepted steps.
//    iwork[7]    =Number of rejected steps.
//    iwork[8]    =Number of evaluations of f used
//                 to estimate the spectral radius
//                 (equal to zero if iwork[0]=1).
//    iwork[9]    =Maximum number of stages used.
//    iwork[10]   =Maximum value of the estimated 
//                 bound for the spectral radius 
//                 (rounded to the nearest integer).
//    iwork[11]   =Minimum value of the estimated  
//                 bound for the spectral radius 
//                 (rounded to the nearest integer).
//                      
// 		iwork[15]   =Number of f function evaluations  
// 		iwork[16]   =Number of g function evaluations
//   Caution:     The variable uround (the rounding unit) is set to 
//   -------      1.0e-16 and may depend on the machines.
//------------------------------------------------------------------
//*** *** *** *** *** *** *** *** *** *** *** *** *** *** *** 
//        Numerical method
//*** *** *** *** *** *** *** *** *** *** *** *** *** *** ***
...
//
//    Data (rec. param.): rec[i]= a_1,a_2,c_2,a_3,c_3,.,a_(ms-2),c_(ms-2)
//                                for ms=1,3,5,..  
//
...
//    g_{s}=ge_{s} -h*sigma*(1-(tau/sigma^2))*(f(g_{s-1}-f(g_{s-2})
//
//    Embedded method: ge_{s}
//
//    Datas (finish. proced.): fp1(ms) =sigma  
//                             fp2(ms) =-sigma(1-(tau/sigma^2))
//                             ms=1,3,5,.. 
//
...
using namespace std;
std::mt19937 rock2::generator;
uniform_int_distribution<> rock2::distribution(1,6);

real get_normal_dist(const real &mean, const real &std_dev){
  std::normal_distribution<double> distribution(mean,std_dev);
	return distribution(rock2::generator);
}


bool operator <(rock2::stoch_sample s1, rock2::stoch_sample s2){return s1.t<s2.t;}

void rock2::check_initialisation(const real& t, const real tend, real& h,
           real atol[], real rtol[], 
           int iwork[], int& idid)
{ 
// -------- Test the initial step size and tolerances.--------     
    ...
    if (iwork[3]==0) //scalar tolerances
    {
        if (atol[0]<=0.0 || rtol[0]<=10.0*uround)
        {
    ...
    //Everything is ok, we integrate.

    return;
}

void rock2::rock2(real& t, const real tend, real& h, real*& y,
              real*& yn, real*& yjm1, real*& yjm2, real*& fn, real*& stowork,
              const real atol[], const real rtol[], 
              int iwork[], int& idid)
{
// ----------------------------------------------
//    Core integrator for ROCK2.
// ---------------------------------------------- 
//             Declarations
// ----------------------------------------------
    real err=0.0;
    ...
    real timebeg,timeend,timeiter=0.;
        
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** *** 
//             Initializations
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** *** 
    if(iwork[5]==0) //if first step
    {
        iwork[4]=0;
        iwork[6]=0;
        iwork[7]=0;
        ...
        //compute first spectral radius
        if (iwork[0]==1)
            eigmax=rho(t,y);
        else //spectral radius approximated via power method
        {
            f(t,y,fn);
            eigmax=rocktrho(t,y,fn,yjm1,yjm2,stowork,idid,iwork);
            if(idid==-3)
                return;
        }
        iwork[10]=(int)eigmax;
...
//    timebeg=get_cpu_time();
    for(;;)
    {
// -------- Initialization of the integration step.--------   
        if(!reject) //if previous step was accepted
        {
            // the solution is put into yn, y becomes a free working space for
            // for the next step
            swap_ptr = yn;
            yn = y;
            y= swap_ptr;
            
            // compute f(t,yn) and put into fn
            f(t,yn,fn);

            iwork[4]=iwork[4]+1; //count f evaluations
						f_evals+=1;
        }

// -------- Step size is adjusted.--------
        if(1.1*abs(h)>=abs(tend-t)) //take abs because h can be negative (when t>tend)
        {
					 if( h < 0 ) printf("h is negative!!");
            h=tend-t;
            last=true; //we are going to do the last integration step
        }
        if(h<10.0*uround)
        {
            cout<<" Tolerances are too small. "<< stoch_err <<endl;
            idid=-2;
            return;
        }
        
// -------- Recompute the spectral radius every 25 steps, if it is not constant
        if(iwork[1]==0 && nrho==0) //nrho==0 every 25 steps
        {
            if (iwork[0]==1)
                eigmax=rho(t,yn);
            else
                eigmax=rocktrho(t,yn,fn,yjm1,yjm2,stowork,idid,iwork);
            if ((int)eigmax>iwork[10]) //update statistics
                iwork[10]=(int)eigmax;
            if ((int)eigmax<iwork[11]) 
                iwork[11]=(int)eigmax;
        }
// -------- The number of stages.--------
        if(iwork[13]==0)//deterministic rock2
            mdeg=(int)sqrt((1.5+h*eigmax)/0.811); //here mdeg=s-1   
        else if(iwork[13]==3) //determinist rkc
            mdeg=(int)sqrt((1.5+h*eigmax)/0.63); //here mdeg=s-1   
        else            //stochastic rock2
            mdeg=(int)sqrt((1.5+h*eigmax)/0.42); //here mdeg=s-1   
        //if we need too much stages the step size is reduced
        if(mdeg>199 && iwork[13]!=3) //s>200
        { 
            if(iwork[13]==0)
                h=25951./eigmax; //25951=0.8*(200*200*0.811-1.5), here s=200
            else
                h=13709./eigmax; //13709=0.8*(202*202*0.42-1.5),  here s=200
            mdeg=198; //mdeg=s-2 with s=200
            last=false; //we have reduced the time step
        }
        else//take at minimum 3 stages
            mdeg=max(mdeg,2)-1; //mdeg=s-2
        
        //at this point s=mdeg+2
        
        //if the number of stages has changed from previous step
        if(mdeg!=mdego)//mp tells us how to find the coefficients of the
            mdegr(mdeg,mp);//polynomials in the tables

        if (mdeg+2>iwork[9])//update statistics
            iwork[9]=mdeg+2;
...
// -------- Computation of an integration step.--------
        ...
        if(iwork[13]==0){
					f_evals+=mdeg+1;
            rtstep_no_noise(t,h,y,yn,fn,yjm1,
                   yjm2,mdeg,mp,err,atol,rtol,iwork[3]);
				}
        else if(iwork[13]==1){
            rtstep_diag_noise(t,h,y,yn,fn,yjm1,
                   yjm2,stowork,mdeg,mp,det_err,stoch_err,atol,rtol,iwork[3]);
						//err=det_err+stoch_err;
						err=det_err+pow(stoch_err,4.0/3);
						//printf("%lf %lf\n",det_err,stoch_err);
						//printf("%d\n",iwork[15]);
						//printf("%d\n",iwork[5]);
						//printf("%lf %lf\n",err,h);
            ...
            rtstep_gen_noise(t,h,y,yn,fn,yjm1,
                   yjm2,mdeg,mp,err,atol,rtol,iwork[3]);
        else if(iwork[13]==3)
            rtstep_no_noise_rkc(t,h,y,yn,fn,yjm1,
                   yjm2,mdeg,err,atol,rtol,iwork[3]);
				//printf("%lf\n",y[2]);
        timeend=get_cpu_time();
        timeiter += timeend-timebeg;
        
        mdego=mdeg;
        iwork[5]=iwork[5]+1; 
        iwork[4]=iwork[4]+mdeg+1;
				
        if(iwork[12]==1) // if time step adaptivity enabled
        {
// -------- Error control procedure.--------
            if(!reject && errp) //if previous step was accepted and we are not
            {                   //at the very first step we compute new step
                facp=1.0/err;   //size using embedded method
                fac=facp*(h/hp);
                fac=errp*fac*fac;
            ...
            else //the previous step was rejected, so we use a more cautious 
                fac=sqrt(1.0/err);//step adaptivity procedure

            fac=min(facmax,max(0.1,0.8*fac));//safety
            hnew=h*fac;
        }
        else //if we want constant step size
            hnew=h;
// -------- Accepted step or without time step adaptivity--------
        if(err<1.0 || iwork[12]==0)
        {
            if(iwork[14])
                cout<<"Step t = "<<t<<" with dt = "<<h<<" and s = "<<mdeg+2<<" ACCEPTED with err = "<<err<<endl;
            
            iwork[6]=iwork[6]+1;
            facmax=2.0;
            t=t+h;
            errp=err;
            if(reject)//if previous step was rejected we are more cautious
            {
                hnew = h>0.0 ? min(hnew,h):max(hnew,h); 
                reject=false; //this step has been accepted
                nrej=0;       //put to zero the number of consecutive rejections
            }
            hp=h; //update step sizes
            h=hnew;
            nrho=nrho+1;
            nrho=(nrho+1)%25;

						// delete sampling history up to accepted timestep - optimization
						real eps=1e-10;
						for ( int i=0;i<neqn;i++){
							while ( !chi_t[i].empty() && chi_t[i].begin()-> t < t-eps ){
								chi_t[i].erase(chi_t[i].begin());
							}
							while ( xi_t[i].empty() && xi_t[i].begin()-> t < t - eps ){
								xi_t[i].erase(xi_t[i].begin());
							}
						}


            if(last) //if were last step
                break;
            else if(iwork[2]==1)//if we want to exit after each step
            {
                idid=2;
                break;
            }
...
// -------- Rejected step.--------
        else
        {
            ...
            if(iwork[14])
                cout<<"Step t = "<<t<<" with dt = "<<h<<" and s = "<<mdeg+2<<" REJECTED with err = "<<err<<endl;
            ...
            h=0.8*hnew;
            facmax=1.0;
            if(iwork[5]==1)//is the first step
                h=0.1*h;
            if(told==t)//previous step was already rejected
            {
                nrej=nrej+1;
                if (nrej==10) 
                    h=h*1e-3;
                ...
                told=t;
// -------The spectral radius is recomputed.--------
...
//    timeend=get_cpu_time();
//    cout<<"Total time "<<timeend-timebeg<<endl;
//    cout<<"Time for steps "<<timeiter<<endl;
    return;
// ----------------------------------------------
//     End of subroutine rockcore.
// ----------------------------------------------  
}        
void rock2::rtstep_no_noise(const real t, const real h, real*& y,
...
//-----------------------------------------------
    ...
    int mr,mz,i,j;
    real *swap_ptr;
    real  *temp_array; // Added by Aleksa
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** *** 
//             Initializations
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** ***
    err=0.0;
    mz=mp[0]; //used to look into tables
    mr=mp[1]; //also
// -------- First stage.--------
    temp1=h*recf[mr-1]; //temp1 = h*mu_1
    ci1=t+temp1;        //ci1 = t+dt*mu_1 = t+dt*c_1
    ci2=ci1;            //ci2 = t+dt*mu_1
    ci3=t;
    
    if(mdeg>1)//more than three stages
    {
        for(i=0;i<neqn;i++)
        {
            yjm2[i]=yn[i];              //yjm2 is K_0
            yjm1[i]=yn[i]+temp1*fn[i];  //yjm1 is K_1
        }
        
        //need to save the address of yjm2 for later
        swap_ptr = yjm2;
     
        // --------- Stages for j = 2 ... mdeg = s-2
        for(j=2;j<=mdeg;j++)
        {
            ...
            //computing righthand side
            f(ci1,yjm1,y); // y = f(t+dt*c_{j-1},yjm1)
            ...
            //computing K_j
            for(i=0;i<neqn;i++)
                y[i]=temp1*y[i]+temp2*yjm1[i]+temp3*yjm2[i]; //y is K_j
...
// -------- Shift the value "y" for the next stage.--------
            if(j<mdeg)
            {
                yjm2 = yjm1;
                yjm1 = y;
                y = swap_ptr;  //becomes free working space
                swap_ptr=yjm2; //save for later
    ...
    // at this point we have y = K_{s-2}, ci1 = t+dt*c_{s-2}
    // and yjm1, yjm2 are usable working spaces
    
// ------ The two-stage finishing procedure (different from one for stochastic)
    temp1=h*fp1[mz-1];  //temp1 = dt*sigma
    temp2=h*fp2[mz-1];  //temp2 = -dt*sigma*(1-tau/sigma^2)
    f(ci1,y,yjm2);
    for(j=0;j<neqn;j++)
        y[j]=y[j]+temp1*yjm2[j]; //y = K_{s-1}
    ci1=ci1+temp1;
    f(ci1,y,yjm1);

    // Doing last stage and estimating local error via embedded formula
    if(ntol==0) // If Atol and rtol are scalars
        for(j=0;j<neqn;j++)
        {
            temp3=temp2*(yjm1[j]-yjm2[j]);
            y[j]=y[j]+temp1*yjm1[j]+temp3;
            ci1=max(abs(y[j]),abs(yn[j]));
            err=err+pow(temp3/(atol[0]+ci1*rtol[0]),2);
        }
// -------- Atol and rtol are array.--------
        ...
        for(j=0;j<neqn;j++)
        {
            ...
            y[j]=y[j]+temp1*yjm1[j]+temp3;
            ci1=max(abs(y[j]),abs(yn[j]));
            err=err+pow(temp3/(atol[j]+ci1*rtol[j]),2);
        }
...
// ----------------------------------------------
//     End of subroutine rtstep.
// ---------------------------------------------- 
}  

void rock2::rtstep_diag_noise( const real t, const real h, real*& y,
...
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** ***
    ...
    const real alpha = recalph[mz];
		...
		// Added by Aleksa		
		// DONE: implement stages 1 to s  
		
    temp1=alpha*h*recf[mr-1]; //temp1 = h*mu_1*alpha
    ci1=t+temp1;        //ci1 = t+dt*mu_1 = t+dt*c_1
    ci2=ci1;            //ci2 = t+dt*mu_1
    ci3=t;
		// not necessary
		
    if(s>1)//more than three stages
    {
        for(i=0;i<neqn;i++)
        {
            yjm2[i]=yn[i];              //yjm2 is K_0
        ...
        // --------- Stages for j = 2 ... mdeg+2 = s-2
        for(j=2;j<=s;j++)
        {
            //updating coefficients
            temp1= alpha*h*recf[mr++];//temp1 = dt*mu_j*alpha
            temp3= -recf[mr++]; //temp3 = -kappa_j
            temp2= 1.0-temp3;   //temp2 = -nu_j
    ...
    // -------- The two-stage finishing procedure for stochastics
    const real &sigma=fp1[mz-1]; 
    temp2=fp2[mz-1];  //temp2 = -sigma*(1-tau/sigma^2)
    real tau = sigma*(temp2+sigma);
		real sigma_a=(1-alpha)/2+alpha*sigma;
		real tau_a=(alpha-1)*(alpha-1)/2+2*alpha*(1-alpha)*sigma+alpha*alpha*tau;
		real sqrt_h=sqrt(h);	
		real sqrt_2=sqrt(0.5);
		// init stochastic variables
		construct_xi(t,h,neqn,stowork);
		construct_chi(t,h,neqn,stowork+neqn);

		// keep on finalization
	  f(ci3,yjm2,stowork+2*neqn); // construct f(K_{s-2})
		for ( int i=0;i<neqn;i++){
			real &xi=stowork[i];
			// we fill stowork
			// stowork[0...neqn[ = K_{s-1}^*
			stowork[3*neqn+i]=yjm2[i]+2*tau_a*h*stowork[2*neqn+i]+g_diag(ci1,y[i],i+1)*xi;
		}
		f(ci3+2*tau_a*h*ci3,stowork+3*neqn,stowork+4*neqn); // construct f(K_{s-1}^*)
		for (int i=0;i<neqn;i++){
			real &xi=stowork[i];
			real &chi=stowork[neqn+i];
			real Jqq=get_Jqq(xi,h);
			// initialize X1 with basic values so we can free stowork
			real gval_s=g_diag(ci1,y[i],i+1);
			y[i]=yjm2[i]+(2*sigma_a-0.5)*h*stowork[2*neqn+i]+0.5*h*stowork[4*neqn+i]
				+0.5     *(g_diag(ci1,y[i]+gval_s*Jqq,i+1)-g_diag(ci1,y[i]-gval_s*Jqq,i+1))
				+0.5*(g_diag(ci2,yjm1[i]+sqrt_2*gval_s*chi,i+1) + g_diag(ci2,yjm1[i]-sqrt_2*gval_s*chi,i+1))*xi;
		}
...
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** ***
    ...
    //temp2=h*fp2[mz-1];  //temp2 = -dt*sigma*(1-tau/sigma^2)
		for ( int i=0;i<neqn;i++){
			...
			// we fill stowork
			// stowork[0...neqn[ = K_{s-1}^*
			stowork[5*neqn+i]=yjm2[i]+2*tau_a*h*stowork[2*neqn+i];
		}	
		f(ci3+2*tau_a*h*ci3,stowork+5*neqn,stowork+6*neqn);
		for ( int i=0;i<neqn;i++){
			temp1=atol[0]+max(abs(y[i]),abs(yn[i]))*rtol[0];
			real &xi=stowork[i];
			det_err+=pow(0.5*h*(1-sigma_a*sigma_a/(tau_a))*abs(stowork[2*neqn+i]-stowork[6*neqn+i])/temp1,2);
		}
		det_err=sqrt(det_err/neqn);
		// Calculate stochastic error
		stoch_err=0.0;
		for ( int i=0;i<neqn;i++){
			real &xi=stowork[i];
			real &chi=stowork[neqn+i];
			...
			temp1=atol[0]+max(abs(y[i]),abs(yn[i]))*rtol[0];
			stoch_err+=pow(abs(
					0.5*(g_diag(ci2,yjm1[i]+0.5*gval_s*sqrt_h,i+1) + g_diag(ci2,yjm1[i]-0.5*gval_s*sqrt_h,i+1)
						-2*g_diag(ci2,yjm1[i],i+1))*xi
						+ 0.5*h*(stowork[6*neqn+i]-stowork[4*neqn+i])
					)/(temp1),2);
		}
		stoch_err=sqrt(stoch_err/neqn);
}  

void rock2::rtstep_gen_noise(const real t, const real h, real*& y,
            real*& yn, real*& fn, real*& yjm1, real*& yjm2,
            int mdeg, int mp[], real& err, const real atol[],
            const real rtol[], const int ntol)
{
...
// *** *** *** *** *** *** *** *** *** *** *** *** *** *** ***
    ...
    ci2=ci1;            //ci2 = t+dt*mu_1
    ci3=t;
		if ( mdeg <= 1 ) mdeg= 2; // always do at least 3 stages
		// not necessary
    if(mdeg>1)//more than three stages
    {
        for(i=0;i<neqn;i++)
    ...
    tau = sigma*(temp2+sigma);
		...
		// init stochastic variables
		real*xi=new real[mdeg];
		real*chi=new real[mdeg];
		real**J_qr=new real*[mdeg];
		for ( int i=0;i<mdeg;i++)
			J_qr[i]=new real[mdeg];
		// Error estimation ( we take just one sample??? )
    err=0.0; 
...
namespace rock2{
	std::set<sp_sample> *chi_t;
	std::set<sp_sample> *xi_t;
}

real rock2::get_Jqq(const real&xi,const real& h){
	real ret_val=(xi*xi-h)/2;
	return ret_val;
}
real fabs(real d){ if ( d < 0 ) return -d ; return d; }
void rock2::construct_xi(const real &t,const real &h,const int &neqn,real *xi){ // added by Aleksa
	...
	real eps=1e-11;
	for ( int i=0;i<neqn;i++){
		s_val=upper_bound(xi_t[i].begin(),xi_t[i].end(),sp_sample(t-eps,0))->val;	

		// bug check 
		if ( fabs(t-upper_bound(xi_t[i].begin(),xi_t[i].end(),sp_sample(t-eps,0))->t)*1000 > 1e-5 ){
			printf("%lf\n",t-upper_bound(xi_t[i].begin(),xi_t[i].end(),sp_sample(t-eps,0))->t);	
		}
		// end 

		// find samples for t0< t < t1
		sp_is1=std::upper_bound(xi_t[i].begin(),xi_t[i].end(),sp_sample(t+h-eps,0));
		while ( sp_is1!= xi_t[i].end() && sp_is1->t < t+ h-eps ){
			sp_is1++;
		//	printf("WTF?! %lf\n",sp_is1->t);
		}
		sp_is0=prev(sp_is1); 
		...
		tmp_val=sqrt(3)*std_dev;

		boost::random::mt19937 rng(2);
		boost::random::uniform_int_distribution<> six(1,6);

		//tmp_val=sqrt(3*h);
		unsigned int seed=12321;
		long long l=(long long)std::chrono::system_clock::now().time_since_epoch().count() ;
		rng.seed( l);

		//printf("%ld\n",std::chrono::system_clock::now());
		int dist_sample=six(rng);
		//int dist_sample=distribution(generator);
		//printf("%d\n",dist_sample);
		if ( dist_sample== 1 ) 
			xi[i]=-tmp_val;
		else if ( dist_sample==2 )
			xi[i]=tmp_val;
		else
			xi[i]=0;
		xi[i]+=exp;
		//xi[i]=get_normal_dist(exp,std_dev);
		xi_t[i].insert(sp_sample(t+h,xi[i]+s_val));
	}	 
}
void rock2::construct_chi(const real &t,const real &h,const int &neqn, real *chi){ // added by Aleksa
	real exp; // expectation
	real std_dev; // standard deviation
	std::set<sp_sample>::iterator sp_is0,sp_is1; 
	real s_val;
	...
	for ( int i=0;i<neqn;i++){
		/*
		s_val=upper_bound(chi_t[i].begin(),chi_t[i].end(),sp_sample(t-eps,0))->val;	

		// find samples for t0< t < t1
		sp_is1=std::upper_bound(chi_t[i].begin(),chi_t[i].end(),sp_sample(t+h-eps,0));
		while ( sp_is1!= chi_t[i].end() && sp_is1->t <= t+ h ){
			sp_is1++;
		//	printf("WTF?! %lf\n",sp_is1->t);
		}
		sp_is0=prev(sp_is1); 
		...
		*/
		...
		if ( dist_sample <=3 ) 
			...
			chi[i]=-tmp_val;
		//chi[i]+=exp; 
		//chi[i]=get_normal_dist(exp,std_dev);
		//chi_t[i].insert(sp_sample(t+h,chi[i]));
	}	
}


void rock2::construct_Jqr(const int &mdeg,const real &h,const real*xi, const real *chi, real**&J_qr){ // changed by Aleksa
	for ( int q=0;q<mdeg;q++)
		for ( int r=0;r<mdeg;r++){
			if ( q== r )
			 	J_qr[q][r]=h*(xi[r]*xi[r]-1)/2;
			else if ( r<q)
				J_qr[q][r]=h*(xi[r]*xi[q]-chi[q])/2;
			else
				J_qr[q][r]=h*(xi[r]*xi[q]+chi[r])/2;
		}
}

bool rock2::operator < (sp_sample s1,sp_sample s2){
	return s1.t < s2.t; 
}


void rock2::rtstep_no_noise_rkc(const real t, const real h, real*& y,
            real*& yn, real*& fn, real*& yjm1, real*& yjm2,
            int mdeg, real& err, const real atol[],
            const real rtol[], const int ntol)
{
//--------------------------------------------------
//  One step of second order RKC
//--------------------------------------------------
    real one=1.;
    real two=2.;
    real four=4.;
    real c13=13.;
    real zero=0.;
    int m=mdeg+2; //number of stages
    
    //working variables
    int i,j;
    real ajm1,arg,bj,bjm1,bjm2,dzj,dzjm1,dzjm2,d2zj,d2zjm1,d2zjm2,mu,mus,nu,
...
//------------------------------
    swap_ptr = yjm2;//save the address of yjm2
    yjm2 = yn;  //yjm2 points to yn
    for(j=2;j<=m;j++)
    {
        //update parameters
        zj   =   two*w0*zjm1 - zjm2;
...
//---------------------------------------------
        f(t + h*thjm1,yjm1,y); // y=f(yjm1)
        for(i=0;i<neqn;i++) //computing stage
...
//          }
    ...
    for(i=0;i<neqn;i++)
    {
        ...
        wt = at + rt*max(abs(y[i]),abs(yn[i]));
        if(wt==zero)
        {
          cerr<<"Error computing error estimation";
          return;
        }
        est = 0.8*(yn[i] - y[i]) + 0.4*h*(fn[i] + yjm1[i]);
        err = err + (est/wt)*(est/wt);
    }      
    err = sqrt(err/neqn);
}
...
//       MP[0]: pointer which select the degree in ms(i)\1,2,..
//             such that mdeg<=ms(i).
//       MP[1]: pointer which gives the corresponding position
...
// -------- Find the degree.--------     
    for(int i=1;i<=46;i++)
    {
        if((ms[i-1]/mdeg)>=1 )// WARNING playing for stochastic part&& ms[i-1] > 5)
        {
          mdeg=ms[i-1];
          mp[0]=i;
          break;
        }
        mp[1]=mp[1]+ms[i-1]*2-1;
    }
// ----------------------------------------------
//     End of subroutine mdegr.
// ----------------------------------------------
}

real rock2::rocktrho(const real t,
                     real *yn, real *fn, real *z, real *fz, real *pz,
                     int& idid, int iwork[])
{
//------------------------------------------------------------ 
//     Rocktrho compute eigmax, a close upper bound of the
...
//             Declarations
//-------------------------------------------------------------
    real eigmax,eigmaxo,sqrtu,znor,ynor,quot,dzyn,dfzfn;
    int i,iter,nind,ntest,ind;  
...
// ------ Perturbation.--------
    for(int i=0;i<neqn;i++)
    {
        ynor=ynor+pow(yn[i],2);
        znor=znor+pow(z[i],2);
    }
    ynor=sqrt(ynor);
    znor=sqrt(znor);
// ------ Normalization of the vector z so that --------
//        the difference z-yn lie in a circle 
//        around yen (ice has a constant modules).
//
...
// ------ Start the power method.--------
    ...
    for(int iter=1;iter<=maxiter;iter++)
    {
        ...
        dfzfn=0.0;
        for(int i=0;i<neqn;i++)
            dfzfn=dfzfn+pow(fz[i]-fn[i],2);
        dfzfn=sqrt(dfzfn);
        eigmaxo=eigmax;
        eigmax=dfzfn/dzyn;
        eigmax=safe*eigmax;
// ------ The stopping criteria is based on a 
//        relative error between two successive
//        estimation ``eigmax'' of the spectral 
//        radius.

        if (iter>=2 && abs(eigmax-eigmaxo)<=(eigmax*0.05))
        {
...
// ----- The next z is defined by --------
//       z_new=yn+coef*(fz-fn) where
//       coef is chosen so that
//       norm(z_new-yn)=norm(z_old-yn).
//
        if (dfzfn!=0.0)
        {
            quot=dzyn/dfzfn;
        ...
        {
// ----- The new z is defined by an arbitrary --------
//       perturbation of the current approximation
//       of the eigenvector.
            nind=neqn;
            ntest=0;
            ind=iter%nind;
...
//  Coefficients for orthogonal polynomials P_i, for i=1,...,s-2.
...
// Final procedure (fp) coefficients
...
// Coefficients for the additional (not needed for deterministic problems)
// orthogonal polynomials P_{s-1} and P_s
real rock2::recf2[184]={0.3005488243231985,0.03229561778843724,0.6512099103020813,0.6392740828523978,0.1881787048433069,0.1294908679614229,0.3692922899497312,0.7942384047165153,0.1266454719228368,0.2166410891457194,
...
real rock2::recalph[46]={1.029217577908589,1.171142615787386,1.289178731889639,1.370039721728061,1.430279163156856,1.478209340802438,1.516334027924159,1.547598809712693,1.573716033722604,1.595931553195606,
1.6150939953885,1.631813398460148,1.646519697230067,1.659614432610494,1.67129422100608,1.681847425139011,1.691412128447207,1.701553522119702,1.709840773771184,1.723292787981514,
